...
bool CellMap::isInMap(int row, int col) const {
    for (Iterator iterator = this->begin(); iterator != this->end(); iterator++)
        if (iterator->second->getRow() == row
                && iterator->second->getCol() == col)
            return true;
    return false;
}

bool CellMap::count(string str) const {
    if(this->cellMap->count(str))
        return true;

    return false;
}

bool CellMap::isInMap(string cell) const {
    for (Iterator iterator = this->begin(); iterator != this->end(); iterator++)
        if (cell.compare(iterator->first) != 0) {
            return true;
        }
    return false;
}
